      ...
      <div style={{maxWidth: "7.5in", fontSize: "10pt"}}>
        ...
        <tbody>
          <tr>
            <td>
              <Page>
                <h1 style={{textAlign: "center", fontWeight: "900"}}>Calibration Summary</h1>
                ...
                <table style={{width: "7.5in", fontSize: "8px", borderCollapse: "collapse"}}>
                  <thead>
                    <tr>
                      <TableHeader><span>Model</span><br /><span> Number</span></TableHeader>
                      <TableHeader><span>Mid-Scale</span><br /><span>Accuracy Fail</span></TableHeader>
                      <TableHeader><span>Visual Inspection</span><span>Fail</span></TableHeader>
                      <TableHeader><span>Hermetic Seal</span><br /><span>Fail</span></TableHeader>
                      <TableHeader><span>Electrical Leakage</span><span>Fail</span></TableHeader>
                      <TableHeader><span>Total Fail</span><br /><span>For Model</span></TableHeader>
                      <TableHeader><span>Total Pass</span><br /><span>For Model</span></TableHeader>
                      <TableHeader><span>Total</span><br /><span>For Model</span></TableHeader>
                    </tr>
                  </thead>
                  <tbody>
                    {uniqueDosimeterModels.map( d => {
                      return(
                        <tr key={d.index} style={{textAlign: "center"}}>
                          ...
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d && c.accPass === false).length}</TableData>
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d && c.vipPass === false).length}</TableData>
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d && c.vacPass === false).length}</TableData>
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d && c.elPass === false).length}</TableData>
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d && c.finalPass === false).length}</TableData>
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d && c.finalPass === true).length}</TableData>
                          <TableData>{calData.filter( c => c.dosimeter.modelNumber === d).length}</TableData>
                        </tr>
                      )}
                    )}
